...
def main():
    ...
    # Sidebar for API Keys - Pulls from st.secrets if available!
    with st.sidebar:
        st.header("🔑 API Credentials")
        st.info("Set these permanently in Streamlit Cloud Secrets, or paste them here.")
        
        with st.expander("Medium", expanded=False):
            medium_token = st.text_input("Medium Integration Token", type="password", value=st.secrets.get("MEDIUM_TOKEN", ""))
        
        with st.expander("Dev.to", expanded=False):
            devto_key = st.text_input("Dev.to API Key", type="password", value=st.secrets.get("DEVTO_KEY", ""))
            
        with st.expander("Hashnode", expanded=False):
            hashnode_pat = st.text_input("Personal Access Token", type="password", value=st.secrets.get("HASHNODE_PAT", ""))
            hashnode_pub_id = st.text_input("Publication ID", value=st.secrets.get("HASHNODE_PUB_ID", ""))
            
        with st.expander("WordPress", expanded=False):
            wp_domain = st.text_input("Site Domain", value=st.secrets.get("WP_DOMAIN", ""))
            wp_token = st.text_input("OAuth Token", type="password", value=st.secrets.get("WP_TOKEN", ""))
            
        with st.expander("Tumblr", expanded=False):
            tumblr_blog = st.text_input("Blog Name", value=st.secrets.get("TUMBLR_BLOG", ""))
            tumblr_c_key = st.text_input("Consumer Key", type="password", value=st.secrets.get("TUMBLR_C_KEY", ""))
            tumblr_c_secret = st.text_input("Consumer Secret", type="password", value=st.secrets.get("TUMBLR_C_SECRET", ""))
            tumblr_o_token = st.text_input("OAuth Token", type="password", value=st.secrets.get("TUMBLR_O_TOKEN", ""))
            tumblr_o_secret = st.text_input("OAuth Secret", type="password", value=st.secrets.get("TUMBLR_O_SECRET", ""))

    # Main Area
    st.write("### Step 1: Paste Your Blog URL")
    target_url = st.text_input("🔗 Target URL", placeholder="https://yourblog.com/testing-tutorial")
